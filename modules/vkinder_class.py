...
class vkinder:
    ...
    def get_photo_user(self, owner_id, num=3):
        # num определяет количество максимально залайканых фото
        method = 'photos.get'
        params = {
            'owner_id': owner_id,
    ...
    def search_users_info(self, search_caunt=15):  # search_caunt - число пользователей в выдаче
        ...
        res = requests.get(url + method, params=params)
        # создаем словарь с именем фамилией, ссылкой на профиль найденых людей
        result_list = []
        for item in res.json()['response']['items']:
            ...
            if owner_res.json()['response'][0]['is_closed']: #проверка приватности страницы
                continue
            time.sleep(0.45) #задержка использования get иначе VK выдаст словарь в 'response' с 'error'
            photo_list = self.get_photo_user(owner_id) #получаем словарь с ссылками на самые популярные фоточки
